...
def inference(
    ...
    C = model.n_cls
    seg_map = torch.zeros((C, ori_shape[0], ori_shape[1]), device=ims.device)
...
class MaskTransformer(nn.Module):
    ...
    def forward(self, x, im_size, return_features=False):
        H, W = im_size
        GS = H // self.patch_size
        ...
        if return_features:
            patches_ = rearrange(patches, "b (h w) n -> b n h w", h=int(GS))
            return masks, patches_
        else:
            return masks
...
class Segmenter(nn.Module):
    ...
    def forward(self, im):
        ...
        masks_body, feats = self.decoder_body(x, (H, W), return_features=True)
        masks_joint = self.decoder_joint(x, (H, W), return_features=False)
        masks = torch.cat([masks_body, masks_joint], dim=1)
...
def create_segmenter_without_cfg(image_size,
                                 patch_size,
                                 n_layers_encoder,
                                 n_layers_decoder,
                                 d_model,
                                 ...
                                 channels: int = 3,
                                 variant: str = None):
    model_args = dict(img_size=image_size, patch_size=patch_size, embed_dim=d_model, depth=n_layers_encoder, num_heads=n_heads, in_chans=channels, dynamic_img_size=True, dynamic_img_pad=True)
    encoder = _create_vision_transformer(variant, pretrained=True, **model_args)
    
    decoder_body = MaskTransformer(2, patch_size, d_model, n_layers_decoder, n_heads, d_model, 4*d_model, drop_path_rate, dropout)
    decoder_joint = MaskTransformer(n_cls-2, patch_size, d_model, n_layers_decoder, n_heads, d_model, 4*d_model, drop_path_rate, dropout)
    model = Segmenter(encoder, decoder_body, decoder_joint, n_cls=n_cls)
...
if __name__ == '__main__':
    m = create_segmenter_without_cfg((224, 224), 16, 12, 2, 384, 6, n_cls=17, variant='vit_small_patch16_224')
